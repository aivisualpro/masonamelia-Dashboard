...
import { getVideos, deleteVideo, deleteVideosByBulk } from "../../api/video.api";
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (confirm.mode === "single") {
        await deleteVideo(confirm.ids[0]);
      } else {
        await deleteVideosByBulk(confirm.ids);
        setSelection([]);
      }
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Stack direction="row" spacing={0.5} alignItems="center">
          <Tooltip title="Edit">
            <IconButton size="small" onClick={() => navigate(`/videos/edit/${params.row.id}`)}>
    ...
    <>
      <div className="flex items-center justify-between px-4 py-3 border-b">
        <div className="flex items-center gap-3">
          <Typography variant="h6" className="font-bold">Videos</Typography>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate("/videos/add")}>
            Add Video
          </Button>
        </div>
      </div>
